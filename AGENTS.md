...
**默认由 `lottery-manager` 接单**，负责：解析用户意图（大乐透 / 双色球 / 快乐八 / 排列5 / 七星彩 × 分析 / 预测）、**盘点本仓库数据文件与期号覆盖**、判断阻塞、**输出分阶段计划与移交清单**（建议下一步应打开的 Subagent 及可复制提示词）。
...
| **七星彩** | 以 `qxc_draws.csv` + `manifest` 为准；字段为 `d1`–`d6`（0-9，允许重复）+ `special`（0-14）。 |

基线随仓库变更；**每次任务**总控应重新盘点。

## 总控接收的典型用户输入

- 大乐透 / 双色球 / 快乐八 / 排列5 / 七星彩：仅分析 / 仅预测 / 分析 + 预测（由总控拆阶段并移交）。
...
| **历史数据分析** | 五彩种描述性统计与数据质量 | `@.cursor/skills/lottery-history-analysis` |
...
| **开奖数据更新（全彩种）** | 含快乐八的三彩种抓取、解析、校验、落盘（当前不含排列5、七星彩） | `@.cursor/skills/lottery-draw-sync` |
...
用户 → lottery-manager（意图 + 数据盘点 + 移交清单）
    ...
    → [按需] 排列5/七星彩按对应 CSV 规范手工补录并更新 manifest（当前无专用 draw Agent）
...
3. **`lottery-prediction`**：同样优先 `data/processed/`；输出 → `history/*_prediction.md`。**强制**：每个彩种归档须含 **「明确号码输出」** 节（大乐透、双色球、排列5、七星彩各 **5 注单式**；快乐八含 20 码/11 码参考，并写预测生成时间与说明），规则见 `lottery-prediction-storage` 与对应 Skill。  
...
- **排列5**：以 **单式多注** 落在 **10～30 元** 内（默认可参考 **5 注单式 ≈10 元**，与 `history/pailie5_prediction.md` 附录一致）；**复式/组选/倍投**仅当用户明确要求。  
- **七星彩**：以 **单式多注** 落在 **10～30 元** 内（前6+后1，2 元/注，可参考 **5 注单式 ≈10 元**）；**直选复式/组选/倍投**仅当用户明确要求，须按官方玩法写出注数公式与小计。  
...
- **历史分析书面归档**：`history/daletou_analysis.md`（大乐透）、`history/shuangseqiu_analysis.md`（双色球）、`history/kuaileba_analysis.md`（快乐八）、`history/pailie5_analysis.md`（排列5）、`history/qixingcai_analysis.md`（七星彩）。**机械统计正文**由 `regenerate-history` + `--only`（`all` / `kl8` / `dlt-ssq` / `pl5` / `qxc`）按**同一默认窗口**刷新对应文件；深度解读与增补由 **`lottery-history-analysis`** 维护；规则见 `.cursor/rules/lottery-history-storage.mdc`。
- **预测参考书面归档**：`history/daletou_prediction.md`、`history/shuangseqiu_prediction.md`、`history/kuaileba_prediction.md`、`history/pailie5_prediction.md`、`history/qixingcai_prediction.md`。由同一 `regenerate-history` 命令按 `--only` 覆盖相应预测 md；专精任务后由 **`lottery-prediction`** 定稿或手写覆盖；规则见 `.cursor/rules/lottery-prediction-storage.mdc`。
