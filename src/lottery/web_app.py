...
from .paths import processed_dir, history_dir
...
def _build_trend_table(lt: str, window: int = 100) -> dict[str, Any]:
   ...
   if lt in ("dlt", "ssq", "kl8"):
       ...
       # 统计计算
       def _calc_stats(draws: list[set[int]], lo: int, hi: int) -> dict:
           n_nums = hi - lo + 1
           freq_win = [0] * (hi + 1)
           for d in draws:
               for v in d:
       ...
       result["main_zone"] = {
           ...
           "stats": _calc_stats(main_draws, m_lo, m_hi),
       }
       ...
       if has_sub:
           result["sub_zone"] = {
               ...
               "stats": _calc_stats(sub_draws, s_lo, s_hi),
           }
       else:
...
def create_app() -> Flask:
   ...
   @app.route("/api/<lt>/prediction-accuracy")
   def api_prediction_accuracy(lt: str):
       if lt not in LOTTERY_META:
           return jsonify({"error": "彩种不存在"}), 404
       try:
           from lottery.db import get_predictions, compute_accuracy
       except Exception:
   ...
   @app.route("/api/<lt>/backtest-summary")
   def api_backtest_summary(lt: str):
       if lt not in LOTTERY_META:
           return jsonify({"error": "彩种不存在"}), 404
       try:
           from lottery.db import get_backtest_results
       except Exception:
           return jsonify({"error": "数据库模块暂不可用"}), 500
   ...
   @app.route("/api/<lt>/backtest-results")
