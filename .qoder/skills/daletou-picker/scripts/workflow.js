...
const AGENTS = [
  ...
  {id:'pattern-spy', role:'形态侦探🔍——审查连号/重号/区间均衡/跨度/同尾，大乐透后区与上期重叠≤1'},
...
for (let round = 1; round <= MAX_ROUNDS && !converged; round++) {
  ...
  const roundReviews = await parallel(
    AGENTS.map(a => () => agent(
      `你是选号审查委员会的**${a.role}**。审查以下大乐透漏斗产出${round>1?'（第'+round+'轮，聚焦未解决争议）':''}。
CSV路径: data/processed/dlt_draws.csv（如需具体数据请用Bash自行读取）
...
log('✅ 完成');
return {funnelResult:pick, final, phases:['结构','形态','精选','对抗验证-收敛循环','终裁','存档']};
