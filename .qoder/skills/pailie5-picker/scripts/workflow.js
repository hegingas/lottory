...
const AGENTS = [
  ...
  {id:'gap-judge', role:'遗漏判官⚖️——审查遗漏状态，标记深冻(>历史最大×0.99)/过热/超跌回补信号，给替换建议'},
...
for (let round = 1; round <= MAX_ROUNDS && !converged; round++) {
  ...
  const roundReviews = await parallel(
    AGENTS.map(a => () => agent(
      `你是选号审查委员会的**${a.role}**。审查以下排列5漏斗产出${round>1?'（第'+round+'轮，聚焦未解决争议）':''}。
CSV路径: data/processed/pl5_draws.csv（如需具体数据请用Bash自行读取）
